...
class list {
 ...
 public:
   ...
   * переиспользовать память, которая уже выделена (Otherwise, the memory owned
   * by *this may be reused when possible.)
   *
  ...
  list& operator=(const list& other) {
    if (this != &other) {
      ...
      // Сначала заполняем уже существующие элементы в текущем списке, пока они
      // не кончатся, либо пока не кончатся элементы в other
   ...
   * *s.begin(). Использование на пустом контейнере UB - никак с этим не боремся
   *
   * @return reference
   */
   ...
   * число, полученное в п.2, на размер одного элемента, т.е. sizeof(node_type)
   *
   ...
   * @details Нельзя использовать на end() - документация запрещает, а так в std
   * можно удалить end() и получить неконсистентный объект
   *
   * @param pos
   */
   ...
   * рассчитан на то, что списки уже отсортированы, поэтому мы экономим время на
   * сортировке результата и результат не будет отсортированным, если списки на
   * входе не отсортированы
   *
   * @param other
   */
    ...
    if (this != &other) {
      ...
      while (this_begin != this_end && other_begin != other_end) {
        if (*other_begin < *this_begin) {
          // Если элемент в other меньше текущего в this, от отцепляем его от
          // other и прицепляем в текущую позицию this
          ...
          // В противном случае сдвигаем итератор this на следующий элемент
          ++this_begin;
        }
      }

      // Оставшиеся в other элементы просто переносим, если в other ещё остались
      // элементы
      splice(end(), other);
    }
  }
   ...
   * только перепривязываются внутренние указатели узлов. Контейнер other всегда
   * становится пустым после операции. Все итераторы и ссылки остаются
   ...
   * оптимизацией. Поэтому наше решение может конкурировать с оригиналом только
   * при компиляции с -O3
   *
   */
   ...
   *
   * Это полезно, потому что независимо от того, насколько умны RVO и семантика
   * перемещения, все еще есть сложные случаи, когда insert, push_back и т.д.,
   * вероятно, сделают ненужные копии.
   *
  ...
  /**
   * @brief Размещает новые элементы args в контейнер непосредственно перед pos.
   * @details Модифицированная версия метода emplace, в целом идейно повторяет
   * emplace() из std (см. описание предыдущей функции), но получает в качестве
   * аргументов args уже созданные объекты для размещения в контейнере (а не
   * параметры для конструирования объектов)
   *
 ...
 private:
   ...
   * равные опорному, больше опорного). На следующий уровень мы передаём только
   * списки меньше опорного и больше опорного.
   *
   * Данное решение мы РЕАЛИЗУЕМ (см. комментарии по коду).
   *
  ...
  void quick_sort(iterator left, iterator right, size_type list_size) {
    ...
    if (left != right && list_size > 1) {
      // Итератор элемента, в который будут свапаться элементы меньше опорного
      iterator swap_iterator = left;
      // Итератор на элемент, являющийся опорным
      iterator pivot_iterator = left;
      // Внутренний экземляр итератора, указывающий на начало обрабатываемого
      // диапазона
      iterator tmp_left = left;
      // Внутренний экземляр итератора, указывающий на конец обрабатываемого
      // диапазона
      iterator tmp_right = right;
      // Сдвигаем итераторы на -1, т.к. это законно всегда (у нас зацикленные
      // итераторы) и этого требует логика нашего алгоритма
      --swap_iterator;
      --pivot_iterator;
      ...
      // Начинаем обход нашего списка, все перемещения элементов делаем обменом
      while (tmp_left != tmp_right) {
        if (*tmp_left < pivot) {
          // Все элементы, меньше опорного, закидываем по порядку в начало
          // списка
          ++swap_iterator;
          ++ishift;
          ...
          // все элементы, равные опорному, закидываем в конец списка, начиная
          // с предпоследнего и движемся влево (последнее место у нас занимает
          // опорный элемент и он в обходе не участвует)
          --tmp_right;
      ...
      // Вызываем рекурсивно эту же функцию для обоих полученных подсписков
      // Если подсписки пустые - ничего страшного, внутри есть проверка на это
  ...
  struct ListNode {
    ...
    /**
     * @brief Конструктор, создающий узел списка, инициализированный значением
     * value
     *
    ...
    /**
     * @brief Конструктор, создающий узел списка, инициализированный значением
     * value, используя move-семантику. Очень актуально для тех случаев, когда
   ...
   *
   * Голова является служебным элементом и присутствует даже у пустого списка. У
   * пустого списка у головы next и  prev указывают на голову.
   *
  ...
  struct ListIterator {
    ...
    /**
     * @brief Префиксное перемещение итератора вперед для обращения к следующему
     * элементу.
     *
    ...
    /**
     * @brief Префиксное перемещение итератора назад для обращения к предыдущему
     * элементу.
     *
     * @return iterator& ссылка на новый элемент
     */
  ...
  /**
   * @brief Класс константного итератора для списка. Используется в тех случаях,
   * когда итератор не должен позволят менять элементы списка, например, для
   * const list
   *
