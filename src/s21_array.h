...
class array {
 ...
 public:
  ...
  array(const array &rhs) noexcept {
    for (size_type i = 0; i < S; ++i) data_[i] = rhs.data_[i];
  }
  ...
  array(array &&rhs) {
    if (this != &rhs) {
      for (size_type i = 0; i < S; ++i) data_[i] = std::move(rhs.data_[i]);
    }
  }
  ...
  constexpr reference front() {
    if (S == 0)
      throw std::out_of_range(
          "s21::array::front Using methods on a zero sized container results "
          "in the UB");
    return data_[0];
  }
