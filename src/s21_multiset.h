...
class multiset {
 public:
  ...
  /**
   * @brief Конструктор копирования (Copy Constructor). Создает мультимножество
  ...
  /**
   * @brief Вставляет элемент со значением value в контейнер. Если в контейнере
  ...
  size_type count(const key_type& key) const {
    ...
    if (*lower_iterator != key) {
      // Если этот элемент не равен key, значит количество таких элементов 0
      return 0;
    }
  ...
  /**
   * @brief Возвращает диапазон, содержащий все элементы с ключом key. Диапазон
   * определяется двумя итераторами, один из которых указывает на первый
   * элемент, который не меньше ключа, а другой указывает на первый элемент
   * больше, чем ключ.
   *
   * Эквивалентно тому, что первый итератор может быть получен с помощью функции
   * lower_bound(), а второй — с помощью функции upper_bound().
   *
