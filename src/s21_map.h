...
class map {
 public:
  ...
  // Компаратор. Для словаря у нас элементы дерева будут считаться равными, если
  // у них равны ключи, значение пары ключ-значение при этом ни на что не
  // влияет.
   ...
   * такого элемента не существует, создается исключение типа std::out_of_range.
   *
   * @param key
   * @return mapped_type&
   */
   ...
   * такого элемента нет, то выполняется вставка нового элемента value_type(key,
   * mapped_type{}).
   *
   ...
   *
   * @param key
   * @return mapped_type& Ссылка на значение нового элемента, если не существует
   * элемента с ключевым ключом. В противном случае ссылка на сопоставленное
   * значение существующего элемента, ключ которого эквивалентен ключу.
   */
  ...
  /**
   * @brief Вставляет элемент c ключом key и со значением obj в контейнер, если
   * контейнер еще не содержит элемент с эквивалентным ключом.
   *
   ...
   * std: 31.847 ms
   * s21: 48.19 ms
   * Вероятно, необходимо избавится от лишнего прохода по дереву (Find - первый
   * проход, InsertUnique - второй)
   * @param key
   * @param obj
